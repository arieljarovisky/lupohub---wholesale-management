...
const MetaAdsCampaigns: React.FC = () => {
  ...
  const kpis = useMemo(
    () => [
      ...
      { label: 'Alcance', value: (summary.reach || 0).toLocaleString('es-AR'), icon: Users, color: 'text-fuchsia-400' },
    ...
    return (
      <div className="max-w-3xl space-y-6">
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm mt-2">
                Un administrador debe cargar el token de acceso y el ID de cuenta publicitaria en{' '}
                <strong className="text-slate-300">Configuración → Integraciones → Meta Ads</strong>, o definir{' '}
                <code className="text-xs text-slate-500">META_ADS_ACCESS_TOKEN</code> y{' '}
                <code className="text-xs text-slate-500">META_ADS_ACCOUNT_ID</code> en el servidor.
              </p>
            </div>
          </div>
        </div>
        <a
          ...
          className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-sm"
        >
          <ExternalLink size={16} /> Abrir Administrador de Meta
        </a>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Facebook e Instagram — métricas por campaña en el período elegido.</p>
        </div>
              ...
              <table className="w-full text-sm text-left min-w-[800px]">
                <thead>
                  <tr className="text-slate-500 border-b border-slate-700/80">
                    ...
                    <th className="py-3 px-4 font-medium">Objetivo</th>
                        ...
                        <td className="py-3 px-4 whitespace-nowrap">
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full ${
                              c.status === 'ACTIVE' ? 'bg-green-500/20 text-green-400' : 'bg-slate-600/30 text-slate-400'
                            }`}
                          >
                            {c.status}
                          </span>
                        </td>
                        <td className="py-3 px-4 text-xs text-slate-400">{c.objective}</td>
                        <td className="py-3 px-4 text-right tabular-nums">${formatMoneyAr(c.spend)}</td>
