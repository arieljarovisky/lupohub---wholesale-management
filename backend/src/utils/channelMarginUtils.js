...
const argentinaDate_1 = require("./argentinaDate");
...
function loadFobMaps(listId) {
    return __awaiter(this, void 0, void 0, function* () {
        const byProductId = new Map();
        const bySku = new Map();
        const rows = (yield (0, db_1.query)(`SELECT pli.product_id, pli.price, p.sku
     FROM price_list_items pli
     LEFT JOIN products p ON p.id = pli.product_id
     WHERE pli.price_list_id = ?`, [listId]));
        for (const r of rows) {
...
function resolveFobPriceListByName(preferredName) {
    return __awaiter(this, void 0, void 0, function* () {
        const wanted = String(preferredName || '').trim();
        if (!wanted)
            return resolveFobPriceList();
        let pl = yield (0, db_1.get)(`SELECT id, name FROM price_lists WHERE LOWER(TRIM(name)) = LOWER(?) LIMIT 1`, [wanted]);
        if (!(pl === null || pl === void 0 ? void 0 : pl.id)) {
            ...
            if (tokens.length > 0) {
                ...
                pl = yield (0, db_1.get)(`SELECT id, name FROM price_lists WHERE ${likes} ORDER BY updated_at DESC, name LIMIT 1`, tokens.map((t) => `%${t}%`));
            }
        }
        if (!(pl === null || pl === void 0 ? void 0 : pl.id)) {
        ...
        const name = String(pl.name || wanted);
...
function resolveFobPriceList() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!id) {
            const month = (0, argentinaDate_1.currentMonthNameEs)();
...
function calcFobYield(revenue, units, fob) {
