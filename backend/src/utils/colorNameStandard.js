...
exports.STANDARD_COLOR_NAMES = void 0;
exports.normalizeColorNameToStandard = normalizeColorNameToStandard;
exports.colorValueNeedsNormalization = colorValueNeedsNormalization;
exports.shouldUpdateColorValue = shouldUpdateColorValue;
exports.isStandardColorName = isStandardColorName;
exports.colorsAreEquivalent = colorsAreEquivalent;
/**
 * Nombres de color estándar para filtros en Tienda Nube.
 */
exports.STANDARD_COLOR_NAMES = [
    'Negro',
    'Blanco',
    'Gris',
    'Azul',
...
function cleanRaw(raw) {
    let v = String(raw !== null && raw !== void 0 ? raw : '').trim();
    if (!v)
        return v;
...
function canonicalFromLeadingColorWord(cleaned) {
    var _a;
    const first = normKey((_a = cleaned.split(/\s+/)[0]) !== null && _a !== void 0 ? _a : '');
    if (!first)
        return null;
    if (ALIASES[first])
        return ALIASES[first];
    for (const name of exports.STANDARD_COLOR_NAMES) {
        if (normKey(name) === first)
            return name;
    }
    return null;
}
 ...
 */
function normalizeColorNameToStandard(raw) {
...
function colorValueNeedsNormalization(raw) {
    const current = String(raw !== null && raw !== void 0 ? raw : '').trim();
    if (!current)
        return false;
    const normalized = normalizeColorNameToStandard(current);
    return normKey(current) !== normKey(normalized) || current !== normalized;
}
function shouldUpdateColorValue(current, normalized) {
    if (!normalized)
        return false;
    if (colorValueNeedsNormalization(current))
        return true;
...
function isStandardColorName(value) {
    const n = normalizeColorNameToStandard(value);
    return exports.STANDARD_COLOR_NAMES.includes(n);
}
 ...
 */
function colorsAreEquivalent(a, b) {
