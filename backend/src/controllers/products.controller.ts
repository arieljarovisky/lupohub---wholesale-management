...
/**
 * Copia los vínculos con ML y TN desde un producto (SKU origen) a otro (SKU destino).
 * Útil cuando cambiaste el SKU en la base de datos y creaste un producto nuevo:
 * si todavía tenés el producto con el SKU viejo (o un backup), podés copiar los vínculos al nuevo.
 * Las variantes se emparejan por talle (size_code) y color (color code).
 * Los vínculos se guardan por product.id y variant.id, no por SKU; si solo actualizaste
 * el SKU en la misma fila, los IDs externos no se borran solos.
 */
export const copyExternalLinksFromSku = async (req: Request, res: Response) => {
  const { fromSku, toSku } = req.body as { fromSku?: string; toSku?: string };
  const from = (fromSku ?? '').toString().trim();
  const to = (toSku ?? '').toString().trim();
  if (!from || !to) {
    return res.status(400).json({
      message: 'Se requiere fromSku y toSku en el body (SKU del producto con vínculos y SKU del producto destino).',
    });
  }
  try {
    const productFrom = await get(
      `SELECT id, sku, tienda_nube_id, mercado_libre_id FROM products WHERE sku = ?`,
      [from]
    );
    const productTo = await get(
      `SELECT id, sku, tienda_nube_id, mercado_libre_id FROM products WHERE sku = ?`,
      [to]
    );
    if (!productFrom) {
      return res.status(404).json({
        message: `No se encontró producto con SKU "${from}". Si ya cambiaste el SKU en la misma fila, los vínculos no se borran solos (están por id). Revisá en la base: SELECT id, sku, tienda_nube_id, mercado_libre_id FROM products WHERE sku = '${to}'.`,
      });
    }
    if (!productTo) {
      return res.status(404).json({ message: `No se encontró producto con SKU "${to}".` });
    }
    if (productFrom.id === productTo.id) {
      return res.json({
        message: 'Origen y destino son el mismo producto. Los vínculos no se modifican.',
    ...
    await execute(
      `UPDATE products SET
         ...
         mercado_libre_id = COALESCE(?, mercado_libre_id)
       WHERE id = ?`,
      [productFrom.tienda_nube_id || null, productFrom.mercado_libre_id || null, productTo.id]
    );

    const variantsFrom = await query(
      `SELECT pv.id, pv.tienda_nube_variant_id, pv.mercado_libre_variant_id, pv.mercado_libre_item_id,
              s.size_code, c.code AS color_code
       FROM product_variants pv
       JOIN product_colors pc ON pc.id = pv.product_color_id
       JOIN products p ON p.id = pc.product_id
       JOIN sizes s ON s.id = pv.size_id
    ...
    const variantsTo = await query(
      `SELECT pv.id, s.size_code, c.code AS color_code
       FROM product_variants pv
       JOIN product_colors pc ON pc.id = pv.product_color_id
       JOIN products p ON p.id = pc.product_id
       JOIN sizes s ON s.id = pv.size_id
    ...
    for (const v of variantsFrom || []) {
      const key = `${(v as any).size_code ?? ''}|${(v as any).color_code ?? ''}`;
      fromByKey.set(key, v);
    }
    let variantsUpdated = 0;
    for (const vTo of variantsTo || []) {
      const key = `${(vTo as any).size_code ?? ''}|${(vTo as any).color_code ?? ''}`;
      const vFrom = fromByKey.get(key);
      if (!vFrom) continue;
      const tn = (vFrom as any).tienda_nube_variant_id;
      const mlVar = (vFrom as any).mercado_libre_variant_id;
      const mlItem = (vFrom as any).mercado_libre_item_id;
      if (!tn && !mlVar && !mlItem) continue;
      await execute(
        `UPDATE product_variants SET
           tienda_nube_variant_id = COALESCE(?, tienda_nube_variant_id),
           mercado_libre_variant_id = COALESCE(?, mercado_libre_variant_id),
           mercado_libre_item_id = COALESCE(?, mercado_libre_item_id)
         WHERE id = ?`,
        [tn || null, mlVar || null, mlItem || null, (vTo as any).id]
      );
      variantsUpdated++;
    }

    res.json({
      message: 'Vínculos copiados correctamente.',
      productId: productTo.id,
      variantsUpdated,
    });
  } catch (error: any) {
    console.error('[copyExternalLinksFromSku]', error);
    res.status(500).json({
      message: 'Error copiando vínculos',
