...
function withRetry429409(fn_1) {
    return __awaiter(this, arguments, void 0, function* (fn, retries = 5) {
        var _a;
        try {
            return yield fn();
        }
...
const ALL_SYNC_PLATFORMS = ['mercadolibre', 'tiendanube'];
// Sincronizar stock a publicaciones vinculadas (variant_publications). Si no hay ninguna, fallback a columnas legacy.
const syncStockToExternalPlatforms = (variantId, newStock, opts) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const result = { mlAttempted: 0, mlOk: 0, tnAttempted: 0, tnOk: 0, logs: [] };
    const platforms = new Set((((_a = opts === null || opts === void 0 ? void 0 : opts.platforms) === null || _a === void 0 ? void 0 : _a.length) ? opts.platforms : ALL_SYNC_PLATFORMS).filter(Boolean));
    try {
        const variantMeta = yield (0, db_1.get)(`SELECT pv.id, COALESCE(pv.external_sku, pv.sku) AS sku,
       ...
       WHERE pv.id = ?`, [variantId]);
        const matchOpts = {
            sku: (variantMeta === null || variantMeta === void 0 ? void 0 : variantMeta.sku) != null ? String(variantMeta.sku) : null,
            color: (variantMeta === null || variantMeta === void 0 ? void 0 : variantMeta.color_name) != null ? String(variantMeta.color_name) : null,
            size: (variantMeta === null || variantMeta === void 0 ? void 0 : variantMeta.size_code) != null ? String(variantMeta.size_code) : null,
            variantId,
            mlVariationId: null,
        };
        ...
        if (publications && publications.length > 0) {
            const { loadVariantMlLinkContext, filterMlPublicationsForSync, filterTnPublicationsForSync } = yield Promise.resolve().then(() => __importStar(require('../services/variantPublicationFilter')));
            const linkCtx = yield loadVariantMlLinkContext(variantId);
            const pubsToSync = linkCtx
                ? [
                    ...(platforms.has('tiendanube') ? filterTnPublicationsForSync(publications, linkCtx) : []),
                    ...(platforms.has('mercadolibre') ? filterMlPublicationsForSync(publications, linkCtx) : [])
                ]
                : publications.filter((p) => platforms.has(p.platform));
            const tasks = [];
            for (const pub of pubsToSync) {
                ...
                if (pub.platform === 'tiendanube' && pub.external_variant_id && platforms.has('tiendanube')) {
                    ...
                    result.tnAttempted++;
                    tasks.push(runExternalSyncWithRetries(label, () => (0, exports.updateTiendaNubeStock)(pub.external_product_id, pub.external_variant_id, stockToSend)).then((ok) => {
                        if (ok)
                            result.tnOk++;
                    ...
                    result.mlAttempted++;
                    tasks.push(runExternalSyncWithRetries(label, () => (0, exports.updateMercadoLibreStockByItem)(itemId, stockToSend, Object.assign(Object.assign({}, matchOpts), { mlVariationId: variationId }))).then((ok) => {
                        if (ok)
                            result.mlOk++;
                        result.logs.push(ok ? `[OK] ${skuLabel} → ${label} = ${stockToSend}` : `[ERROR] ${skuLabel} → ${label}`);
                    }));
         ...
         WHERE pv.id = ?`, [variantId]);
            if (variant) {
                ...
                if (needLegacyTn && variant.tienda_nube_id && variant.tienda_nube_variant_id) {
                    ...
                    result.tnAttempted++;
                    const ok = yield runExternalSyncWithRetries(label, () => (0, exports.updateTiendaNubeStock)(variant.tienda_nube_id, variant.tienda_nube_variant_id, stockTN));
                    if (ok)
                        result.tnOk++;
                ...
                const ownMlItemId = variant.mercado_libre_item_id != null && String(variant.mercado_libre_item_id).trim() !== ''
                    ...
                    : null;
                const parentMlId = variant.mercado_libre_id != null && String(variant.mercado_libre_id).trim() !== ''
                ...
                if (needLegacyMl) {
                    if (ownMlItemId) {
                        ...
                        result.mlAttempted++;
                        const ok = yield runExternalSyncWithRetries(label, () => (0, exports.updateMercadoLibreStockByItem)(ownMlItemId, stockML, matchOpts));
                        if (ok)
                            result.mlOk++;
                        ...
                        result.mlAttempted++;
                        const ok = yield runExternalSyncWithRetries(label, () => (0, exports.updateMercadoLibreStockByItem)(parentMlId, stockML, matchOpts));
                        if (ok)
                            result.mlOk++;
                        result.logs.push(ok ? `[OK] ${skuLabel} → ${label} = ${stockML}` : `[ERROR] ${skuLabel} → ${label}`);
                    }
        ...
        result.logs.push(`[ERROR] variant=${variantId}: ${(error === null || error === void 0 ? void 0 : error.message) || error}`);
    }
    return result;
});
...
const updateMercadoLibreStockByItem = (itemId, stock, opts) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    ...
    try {
        ...
        const getRes = yield withRetry429409(() => axios_1.default.get(`https://api.mercadolibre.com/items/${resolvedItemId}?include_attributes=all`, { headers }));
        ...
        const { matchMlVariationForVariantLink, enrichMlItemVariationsForMatch, mlVariationSkuFromApi, skusCompatible, } = yield Promise.resolve().then(() => __importStar(require('../utils/mlVariationMatch')));
        ...
        const missingSku = variations.some((v) => !mlVariationSkuFromApi(v));
        if (missingSku || (opts === null || opts === void 0 ? void 0 : opts.sku) || (opts === null || opts === void 0 ? void 0 : opts.color) || (opts === null || opts === void 0 ? void 0 : opts.size)) {
            item = yield enrichMlItemVariationsForMatch(item, integration.access_token, (url, cfg) => withRetry429409(() => axios_1.default.get(url, cfg)));
            variations = item.variations || [];
        }
        const matched = matchMlVariationForVariantLink(variations, {
            variationId: opts === null || opts === void 0 ? void 0 : opts.mlVariationId,
            sku: opts === null || opts === void 0 ? void 0 : opts.sku,
            color: opts === null || opts === void 0 ? void 0 : opts.color,
            size: opts === null || opts === void 0 ? void 0 : opts.size,
        });
        if (!(matched === null || matched === void 0 ? void 0 : matched.id)) {
            console.log(`[ML Stock] Item ${resolvedItemId} tiene ${variations.length} variaciones y no se pudo emparejar` +
                ((opts === null || opts === void 0 ? void 0 : opts.sku) ? ` por SKU ${opts.sku}` : ' (sin SKU/atributos)') +
                ((opts === null || opts === void 0 ? void 0 : opts.mlVariationId) ? ` (variation_id=${opts.mlVariationId} no está en el ítem)` : ''));
            ...
            if ((opts === null || opts === void 0 ? void 0 : opts.variantId) && (opts === null || opts === void 0 ? void 0 : opts.mlVariationId)) {
                const idStillOnItem = variations.some((v) => String(v === null || v === void 0 ? void 0 : v.id) === String(opts.mlVariationId));
                if (!idStillOnItem) {
                    try {
                        yield (0, db_1.execute)(`UPDATE product_variants SET mercado_libre_variant_id = NULL WHERE id = ? AND mercado_libre_variant_id = ?`, [opts.variantId, String(opts.mlVariationId)]);
                    }
                    catch (_b) {
                        /* best-effort */
                    }
        ...
        const skuExact = !!(opts === null || opts === void 0 ? void 0 : opts.sku) && !!remoteSku && skusCompatible(String(opts.sku), remoteSku);
        const linkedBySavedId = !!(opts === null || opts === void 0 ? void 0 : opts.mlVariationId) && String(opts.mlVariationId) === variationId;
        // Persistir variation_id si match por SKU exacto o si el usuario ya lo tenía vinculado.
        if ((opts === null || opts === void 0 ? void 0 : opts.variantId) && (skuExact || linkedBySavedId)) {
            try {
                yield (0, db_1.execute)(`UPDATE product_variants SET mercado_libre_variant_id = ? WHERE id = ?`, [variationId, opts.variantId]);
            }
            catch (_c) {
                /* backfill best-effort */
            }
        }
        console.log(`[ML Stock] Ítem multi-variación ${resolvedItemId}: emparejado SKU=${(opts === null || opts === void 0 ? void 0 : opts.sku) || '-'} → var ${variationId}` +
            (skuExact ? ' (SKU exacto)' : linkedBySavedId ? ' (variation_id guardado)' : ''));
        return (0, exports.updateMercadoLibreStockByVariant)(resolvedItemId, variationId, stock);
    }
    catch (e) {
...
const updateVariantStockEndpoint = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        let userName = typeof (user === null || user === void 0 ? void 0 : user.name) === 'string' ? user.name : null;
        if (!userName && userId !== 'sistema') {
            const row = yield (0, db_1.get)(`SELECT name FROM users WHERE id = ?`, [userId]);
            userName = (row === null || row === void 0 ? void 0 : row.name) ? String(row.name) : null;
        }
        const reference = userName
            ? `Ajuste por usuario: ${userName}`
            : `Ajuste por usuario ${userId}`;
        const ok = yield (0, exports.updateVariantStock)(variantId, Math.floor(stock), 'AJUSTE_MANUAL', reference, true);
        if (!ok)
...
const importStockFromExcel = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        for (const row of rawRows) {
            ...
            if (/^\d{4}$/.test(colorStr))
                colorStr = colorStr.slice(0, 3);
            if (!codigo || !colorStr)
                continue;
