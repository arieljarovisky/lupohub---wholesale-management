...
exports.resolveMercadoLibreItemsByFamilyName = resolveMercadoLibreItemsByFamilyName;
exports.resolveMercadoLibreItemsByArticlePrefix = resolveMercadoLibreItemsByArticlePrefix;
...
const tiendanubeExpressTrackingPage_service_1 = require("../services/tiendanubeExpressTrackingPage.service");
const mlVariationMatch_1 = require("../utils/mlVariationMatch");
const stockSyncJob_service_1 = require("../services/stockSyncJob.service");
...
function mlOrderLineReference(orderId, mlItemId, mlVariationId, itemIndex) {
    const item = mlItemId != null && String(mlItemId).trim() !== '' ? String(mlItemId).trim() : 'item';
    const variation = mlVariationId != null && String(mlVariationId).trim() !== '' ? String(mlVariationId).trim() : '0';
...
function isMlOrderLineAlreadyProcessed(orderId, lineRef, variantId) {
    return __awaiter(this, void 0, void 0, function* () {
        const byLine = yield (0, db_1.get)(`SELECT id FROM stock_movements WHERE movement_type = 'VENTA_MERCADO_LIBRE' AND reference = ? LIMIT 1`, [lineRef]);
        if (byLine)
            return true;
        if (variantId) {
            const legacyRef = `Orden ML: ${orderId}`;
            const byLegacyVariant = yield (0, db_1.get)(`SELECT id FROM stock_movements WHERE movement_type = 'VENTA_MERCADO_LIBRE' AND reference = ? AND variant_id = ? LIMIT 1`, [legacyRef, variantId]);
            if (byLegacyVariant)
                return true;
        }
        return false;
...
function dedupeMlVariationRows(rows) {
    var _a;
    const byKey = new Map();
    for (const row of rows) {
        ...
        const rawSku = String((_a = row.sku) !== null && _a !== void 0 ? _a : '').trim();
        const skuKey = rawSku ? (0, mlVariationMatch_1.normSkuForMlStockMatch)(rawSku) : '';
        const colorN = (0, mlVariationMatch_1.normTextForMlStockMatch)(row.color);
        const sizeN = (0, mlVariationMatch_1.normTextForMlStockMatch)(row.size);
...
function extractMlVariationsFromItemData(it) {
    ...
    const rawColor = (colorAttr ? ((_f = (_e = colorAttr.value_name) !== null && _e !== void 0 ? _e : colorAttr.value) !== null && _f !== void 0 ? _f : '') : parsed.color).toString().trim();
    const rawSize = (sizeAttr ? ((_h = (_g = sizeAttr.value_name) !== null && _g !== void 0 ? _g : sizeAttr.value) !== null && _h !== void 0 ? _h : '') : parsed.size).toString().trim();
    const reconciled = (0, mlVariationMatch_1.reconcileMlColorSizeWithLupoSku)(sku, rawColor, rawSize);
    out.push({
...
function mlVariationRowFromApiVariation(v, item) {
    const { color, size } = (0, mlVariationMatch_1.mlVariationColorSizeFromApi)(v);
    const sku = (0, mlVariationMatch_1.mlVariationSkuFromApi)(v);
    ...
    const reconciled = (0, mlVariationMatch_1.reconcileMlColorSizeWithLupoSku)(sku, color || parsed.color, size || parsed.size);
    return {
...
function enrichMercadoLibreItemVariationsForCatalog(item, accessToken) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const needsEnrich = (v) => {
            ...
            if (!Array.isArray(ac) || ac.length === 0)
                return true;
            const { color, size } = (0, mlVariationMatch_1.mlVariationColorSizeFromApi)(v);
            return !(0, mlVariationMatch_1.mlVariationSkuFromApi)(v) && !color && !size;
        };
...
const getIntegrationStatus = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const metaAds = integrations.find((i) => i.platform === 'meta_ads');
        const googleAds = integrations.find((i) => i.platform === 'google_ads');
        const status = {
            ...
            metaAds: !!((metaAds === null || metaAds === void 0 ? void 0 : metaAds.access_token) && (metaAds === null || metaAds === void 0 ? void 0 : metaAds.user_id)) || !!(process.env.META_ADS_ACCESS_TOKEN && process.env.META_ADS_ACCOUNT_ID),
            googleAds: !!((googleAds === null || googleAds === void 0 ? void 0 : googleAds.refresh_token) && (googleAds === null || googleAds === void 0 ? void 0 : googleAds.user_id)) || !!(process.env.GOOGLE_ADS_REFRESH_TOKEN && process.env.GOOGLE_ADS_CUSTOMER_ID),
        };
        res.json(status);
    }
    catch (error) {
...
const handleTiendaNubeCallback = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    `, [access_token, response.data.refresh_token || null, expiresAt, user_id, user_id]);
        ...
        try {
            const pageCfg = yield (0, tiendanubeExpressTrackingPage_service_1.loadExpressTrackingPageConfig)();
            if (pageCfg.enabled) {
                yield (0, tiendanubeExpressTrackingPage_service_1.syncExpressTrackingPageToStore)({ enabled: true });
                console.log('[TN] Página de seguimiento express sincronizada tras reconexión OAuth.');
            }
        }
        catch (pageErr) {
            console.warn('[TN] No se pudo sincronizar página de seguimiento tras OAuth:', (pageErr === null || pageErr === void 0 ? void 0 : pageErr.message) || pageErr);
        }
...
const processTiendaNubeOrder = (orderId) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const { deductVariantStockForChannelSale } = yield Promise.resolve().then(() => __importStar(require('./stock.controller')));
        ...
        for (const item of order.products || []) {
            ...
            if (tnProductId || tnVariantId) {
                ...
                if ((_c = bundle === null || bundle === void 0 ? void 0 : bundle.items) === null || _c === void 0 ? void 0 : _c.length) {
                    const { ok, blocked, lines } = yield deductStockForBundleListing(bundle, quantity, 'VENTA_TIENDA_NUBE', `Orden TN: ${orderId}`);
                    if (blocked) {
                        console.warn(`[TN Order] Pack multicolor BLOQUEADO (stock insuficiente) "${bundle.label || bundle.externalProductId}": ${lines.join('; ')}`);
                    }
                    else if (ok) {
                        discountedCount++;
           ...
           WHERE p.sku = ? OR pv.sku LIKE ? OR pv.external_sku = ?`, [itemSku, `${itemSku}%`, itemSku]);
            ...
            if (variant === null || variant === void 0 ? void 0 : variant.id) {
                ...
                const result = yield deductVariantStockForChannelSale(variant.id, unitsToDeduct, 'VENTA_TIENDA_NUBE', `Orden TN: ${orderId}`, true);
                if (result.blocked) {
                    console.warn(`[TN Order] BLOQUEADO: stock insuficiente variante ${variant.id} (tiene ${result.previousStock}, pide ${result.unitsToDeduct}) orden ${orderId}`);
                }
                else if (result.ok) {
                    discountedCount++;
                    console.log(`[TN Order] Descontado ${result.unitsToDeduct} un. (${quantity} × pack x${tnPack}) variante ${variant.id}, stock: ${result.previousStock} -> ${result.newStock}; actualizado ML y TN`);
                }
                else {
...
const testMercadoLibreOrder = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const before = yield (0, db_1.get)(`SELECT COUNT(*) AS n FROM stock_movements WHERE movement_type = 'VENTA_MERCADO_LIBRE' AND reference LIKE ?`, [refPrefix]);
        yield processMercadoLibreOrder(orderId);
        const after = yield (0, db_1.get)(`SELECT COUNT(*) AS n FROM stock_movements WHERE movement_type = 'VENTA_MERCADO_LIBRE' AND reference LIKE ?`, [refPrefix]);
        ...
        for (const [itemIndex, item] of (order.order_items || []).entries()) {
            ...
            if (mlItemId) {
                ...
                if ((_d = bundle === null || bundle === void 0 ? void 0 : bundle.items) === null || _d === void 0 ? void 0 : _d.length) {
                    if (yield isMlOrderLineAlreadyProcessed(orderId, lineRef)) {
                        console.log(`[ML Order] Línea ${itemIndex + 1} de orden ${orderId} ya procesada (pack), omitiendo`);
                        continue;
                    }
                    const { ok, blocked, lines } = yield deductStockForBundleListing(bundle, quantity, 'VENTA_MERCADO_LIBRE', lineRef);
                    if (blocked) {
                        console.warn(`[ML Order] Pack multicolor BLOQUEADO (stock insuficiente) "${bundle.label || bundle.externalProductId}": ${lines.join('; ')}`);
                    }
                    else if (ok) {
           ...
           WHERE pv.mercado_libre_variant_id = ?`, [mlVariationId]);
            ...
            if (!(variant === null || variant === void 0 ? void 0 : variant.id) && mlItemId && !mlVariationId) {
                variant = yield (0, db_1.get)(`SELECT pv.id, s.stock AS current_stock, COALESCE(NULLIF(p.mercado_libre_pack_size, 0), 1) AS ml_pack
           FROM product_variants pv
           JOIN product_colors pc ON pc.id = pv.product_color_id
           JOIN products p ON p.id = pc.product_id
           ...
           WHERE p.sku = ? OR pv.sku LIKE ? OR pv.external_sku = ?`, [itemSku, `${itemSku}%`, itemSku]);
            ...
            if (variant === null || variant === void 0 ? void 0 : variant.id) {
                if (yield isMlOrderLineAlreadyProcessed(orderId, lineRef, variant.id)) {
                ...
                const result = yield deductVariantStockForChannelSale(variant.id, unitsToDeduct, 'VENTA_MERCADO_LIBRE', lineRef, true);
                if (result.blocked) {
                    console.warn(`[ML Order] BLOQUEADO: stock insuficiente variante ${variant.id} (tiene ${result.previousStock}, pide ${result.unitsToDeduct}) orden ${orderId}`);
                }
                else if (result.ok) {
                    console.log(`[ML Order] Descontado ${result.unitsToDeduct} un. (${quantity} × pack x${mlPack}) variante ${variant.id}, stock: ${result.previousStock} -> ${result.newStock}; actualizado ML y TN`);
                }
                else {
...
function runAutoSyncMLtoTN() {
  ...
  `);
        ...
        // Empareja por ml_variant_id (columnas o variant_publications) / SKU / color+talle.
        const rowsByItem = yield (0, db_1.query)(`
    SELECT pv.id AS variant_id,
           pv.mercado_libre_item_id AS ml_item_id,
           COALESCE(
             NULLIF(TRIM(pv.mercado_libre_variant_id), ''),
             (
               SELECT NULLIF(TRIM(vp.external_variant_id), '')
               FROM variant_publications vp
               WHERE vp.variant_id = pv.id
                 AND vp.platform = 'mercadolibre'
                 AND vp.external_product_id = pv.mercado_libre_item_id
                 AND NULLIF(TRIM(vp.external_variant_id), '') IS NOT NULL
  ...
  `);
        if (rowsByItem === null || rowsByItem === void 0 ? void 0 : rowsByItem.length) {
            ...
            for (let i = 0; i < rowsByItem.length; i += batchSize) {
                ...
                const itemPromises = batch.map((row) => axios_1.default.get(`https://api.mercadolibre.com/items/${row.ml_item_id}?include_attributes=all`, {
                    headers: { 'Authorization': `Bearer ${mlToken.access_token}` }
                }).then(r => r.data).catch(() => null));
                const items = yield Promise.all(itemPromises);
                for (let j = 0; j < batch.length; j++) {
                    ...
                    let mlQty;
                    if (variations.length === 0) {
                        mlQty = (_e = item.available_quantity) !== null && _e !== void 0 ? _e : 0;
                    }
                    else if (variations.length === 1) {
                        mlQty = (_f = variations[0].available_quantity) !== null && _f !== void 0 ? _f : 0;
                    }
                    else {
                        let itemForMatch = item;
                        try {
                            itemForMatch = yield (0, mlVariationMatch_1.enrichMlItemVariationsForMatch)(item, mlToken.access_token, (url, cfg) => axios_1.default.get(url, cfg));
                        }
                        catch (_k) {
                            /* best-effort */
                        }
                        const matched = (0, mlVariationMatch_1.matchMlVariationForVariantLink)(itemForMatch.variations || variations, {
                        ...
                        if (!matched) {
                            console.warn(`[AutoSync ML→TN] Omitido ml_item ${r.ml_item_id} (SKU ${r.sku}): ${variations.length} variaciones y no se pudo emparejar por id/SKU/atributos.`);
                            continue;
                        }
                        mlQty = (_g = matched.available_quantity) !== null && _g !== void 0 ? _g : 0;
                        const matchedVarId = String(matched.id);
                        const remoteSku = (0, mlVariationMatch_1.mlVariationSkuFromApi)(matched).trim();
                        const skuExact = !!(r.sku && remoteSku && (0, mlVariationMatch_1.skusCompatible)(String(r.sku), remoteSku));
                        if (r.variant_id && matchedVarId && skuExact) {
                            try {
                                yield (0, db_1.execute)(`UPDATE product_variants SET mercado_libre_variant_id = ? WHERE id = ?`, [matchedVarId, r.variant_id]);
                            }
                            catch (_l) {
                    ...
                    try {
                        ...
                        errors++;
                        console.warn(`[AutoSync ML→TN] Error TN PUT ítem propio ml_item=${r.ml_item_id} tn=${r.tn_id}/${r.tn_variant_id} (SKU ${r.sku}):`, ((_j = (_h = e.response) === null || _h === void 0 ? void 0 : _h.data) === null || _j === void 0 ? void 0 : _j.message) || e.message);
                    }
                    if (TN_RATE_LIMIT_DELAY_MS > 0)
                        yield sleep(TN_RATE_LIMIT_DELAY_MS);
                }
  ...
  `);
        ...
        for (const r of pendingMlOwn) {
            ...
            if (variations.length > 1) {
                const matched = (0, mlVariationMatch_1.matchMlVariationForVariantLink)(variations, {
                    variationId: r.ml_variant_id,
                    sku: r.variant_sku,
                    color: r.color_name,
                    size: r.size_code,
                ...
                if (!matched) {
                    pushIssue(r, syncMode, 'ML_MULTI_VARIACIONES', `El ítem ML ${r.ml_item_id} tiene ${variations.length} variaciones y no se pudo emparejar por id/SKU/atributos para sincronizar stock a TN.`);
                    continue;
                }
            }
            const tnOk = yield tnVariantExists(String(r.tn_id), String(r.tn_variant_id));
...
const getTiendaNubeStockSyncStatus = (_req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const job = (0, stockSyncJob_service_1.getStockSyncJob)('tn');
    res.json({
        async: true,
        status: job.status,
        message: job.message,
...
exports.getTiendaNubeStockSyncStatus = getTiendaNubeStockSyncStatus;
const syncAllStockToTiendaNube = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        if ((0, stockSyncJob_service_1.isStockSyncRunning)('tn')) {
            const job = (0, stockSyncJob_service_1.getStockSyncJob)('tn');
            return res.json({
                async: true,
                status: 'running',
                message: 'Ya hay un sync a Tienda Nube en curso',
                updated: job.updated,
                errors: job.errors,
                total: job.total,
                logs: job.logs,
        ...
        if (!(0, stockSyncJob_service_1.beginStockSyncJob)('tn', 'Sincronizando stock a Tienda Nube…')) {
        ...
        res.status(202).json({
            async: true,
            status: 'running',
            message: 'Sync a Tienda Nube iniciado en segundo plano',
            updated: 0,
            errors: 0,
            logs: [],
            failuresCount: 0,
        ...
        `);
                ...
                for (const v of variants) {
                    ...
                    try {
                        ...
                        errors++;
                        const reason = ((_b = (_a = e.response) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.description) || ((_d = (_c = e.response) === null || _c === void 0 ? void 0 : _c.data) === null || _d === void 0 ? void 0 : _d.message) || e.message || 'Error desconocido';
                ...
                console.log(`[Sync→TN] Fin: ${updated} OK, ${errors} errores, total=${variants.length}`);
                (0, stockSyncJob_service_1.finishStockSyncJob)('tn', {
                    updated,
                    errors,
                ...
                (0, stockSyncJob_service_1.failStockSyncJob)('tn', (error === null || error === void 0 ? void 0 : error.message) || 'Error sincronizando stock a Tienda Nube');
            }
        }))();
    }
    catch (error) {
        console.error('Error syncing stock to TN:', error);
        (0, stockSyncJob_service_1.failStockSyncJob)('tn', (error === null || error === void 0 ? void 0 : error.message) || 'Error sincronizando stock');
        if (!res.headersSent) {
...
// Usa syncStockToExternalPlatforms (variant_publications + legacy) para no ignorar vínculos del Paso 1.
const syncSelectedStockToTiendaNube = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const variants = yield (0, db_1.query)(`SELECT pv.id, s.stock, COALESCE(pv.external_sku, pv.sku) AS sku
       FROM product_variants pv
       LEFT JOIN stocks s ON s.variant_id = pv.id
       WHERE pv.id IN (${placeholders})`, variantIds);
        ...
        for (const v of variants) {
            const stockHub = Number(v.stock || 0);
            const syncRes = yield syncStockToExternalPlatforms(String(v.id), stockHub, { platforms: ['tiendanube'] });
            logs.push(...syncRes.logs);
            updated += syncRes.tnOk;
            errors += Math.max(0, syncRes.tnAttempted - syncRes.tnOk);
            if (syncRes.tnAttempted === 0) {
                logs.push(`[INFO] ${v.sku || v.id}: sin vínculo TN (columnas ni publicaciones)`);
            }
            if (TN_RATE_LIMIT_DELAY_MS > 0)
                yield sleep(TN_RATE_LIMIT_DELAY_MS);
        }
        const found = new Set(variants.map((v) => String(v.id)));
        const skipped = variantIds.filter((id) => !found.has(id)).length;
        if (skipped > 0)
            logs.push(`[INFO] ${skipped} variante(s) no encontradas, omitidas.`);
        res.json({
...
const getMercadoLibreStockSyncStatus = (_req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const job = (0, stockSyncJob_service_1.getStockSyncJob)('ml');
    res.json({
        async: true,
        status: job.status,
        message: job.message,
...
exports.getMercadoLibreStockSyncStatus = getMercadoLibreStockSyncStatus;
/** Excel con artículos/variantes que no se actualizaron en el último sync masivo. */
const exportStockSyncFailuresXlsx = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const raw = String(req.query.platform || 'both').toLowerCase();
        const platform = raw === 'ml' || raw === 'tn' || raw === 'both' ? raw : 'both';
        const failures = (0, stockSyncJob_service_1.getStockSyncFailures)(platform);
...
exports.exportStockSyncFailuresXlsx = exportStockSyncFailuresXlsx;
...
const syncAllStockToMercadoLibre = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        if ((0, stockSyncJob_service_1.isStockSyncRunning)('ml')) {
            const job = (0, stockSyncJob_service_1.getStockSyncJob)('ml');
            return res.json({
                async: true,
                status: 'running',
                message: 'Ya hay un sync a Mercado Libre en curso',
                updated: job.updated,
                errors: job.errors,
                total: job.total,
                logs: job.logs,
        ...
        if (!(0, stockSyncJob_service_1.beginStockSyncJob)('ml', 'Sincronizando stock a Mercado Libre…')) {
        ...
        res.status(202).json({
            async: true,
            status: 'running',
            message: 'Sync a Mercado Libre iniciado en segundo plano',
            updated: 0,
            errors: 0,
            logs: [],
            failuresCount: 0,
        ...
        void (() => __awaiter(void 0, void 0, void 0, function* () {
            try {
                const { updateMercadoLibreStockByItem } = yield Promise.resolve().then(() => __importStar(require('./stock.controller')));
                const variants = yield (0, db_1.query)(`
        ...
        `);
                ...
                const syncOne = (v) => __awaiter(void 0, void 0, void 0, function* () {
                    ...
                    if (v.mercado_libre_item_id) {
                        ok = yield updateMercadoLibreStockByItem(v.mercado_libre_item_id, stockToSend, matchOpts);
                    }
                    else if (v.mercado_libre_id && v.mercado_libre_variant_id) {
                        ok = yield updateMercadoLibreStockByItem(v.mercado_libre_id, stockToSend, matchOpts);
                    }
                ...
                const workers = Array.from({ length: Math.min(concurrencySafe, Math.max(list.length, 1)) }, () => __awaiter(void 0, void 0, void 0, function* () {
                    while (true) {
                        const i = next++;
                        if (i >= list.length)
                            break;
                        outcomes[i] = yield syncOne(list[i]);
                    }
                }));
                ...
                (0, stockSyncJob_service_1.finishStockSyncJob)('ml', {
                    updated,
                    errors,
                    total: list.length,
                    logs,
                ...
                (0, stockSyncJob_service_1.failStockSyncJob)('ml', (error === null || error === void 0 ? void 0 : error.message) || 'Error sincronizando stock a Mercado Libre');
            }
        }))();
    }
    catch (error) {
...
// Usa syncStockToExternalPlatforms para respetar variant_publications (variation_id / pack).
const syncSelectedStockToMercadoLibre = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
       ...
       WHERE pv.id IN (${placeholders})`, variantIds);
        ...
        for (const v of variants) {
            const stockHub = Number(v.stock || 0);
            const syncRes = yield syncStockToExternalPlatforms(String(v.id), stockHub, { platforms: ['mercadolibre'] });
            logs.push(...syncRes.logs);
            updated += syncRes.mlOk;
            errors += Math.max(0, syncRes.mlAttempted - syncRes.mlOk);
            if (syncRes.mlAttempted === 0) {
                logs.push(`[INFO] ${v.sku || v.id}: sin vínculo ML (columnas ni publicaciones)`);
            }
        }
        const found = new Set(variants.map((v) => String(v.id)));
        const skipped = variantIds.filter((id) => !found.has(id)).length;
...
const getVariantExternalStocks = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
       ...
       WHERE pv.id IN (${placeholders})`, variantIds);
        const { shouldSyncMlPublication } = yield Promise.resolve().then(() => __importStar(require('../services/variantPublicationFilter')));
        const mlItemsByProduct = new Map();
        ...
        for (const r of rows || []) {
            ...
            const ownItemId = r.mercado_libre_item_id != null && String(r.mercado_libre_item_id).trim() !== ''
                ...
                : null;
            if (ownItemId) {
                if (!mlItemsByProduct.has(productId))
                    mlItemsByProduct.set(productId, new Set());
                mlItemsByProduct.get(productId).add(ownItemId);
            }
            variantLinkMeta.set(variantId, {
                productId,
                ownItemId,
                ownVarId: r.mercado_libre_variant_id != null && String(r.mercado_libre_variant_id).trim() !== ''
                    ? String(r.mercado_libre_variant_id).trim()
                    : null,
                parentItemId: r.mercado_libre_id != null && String(r.mercado_libre_id).trim() !== ''
                    ? String(r.mercado_libre_id).trim()
                    : null,
        ...
        for (const [variantId, meta] of variantLinkMeta) {
            ...
            if (!productItems)
                continue;
            meta.siblingOwnItemIds = new Set([...productItems].filter((itemId) => itemId !== meta.ownItemId));
            variantLinkMeta.set(variantId, meta);
        }
        const pubsByVariant = new Map();
        try {
            const pubRows = yield (0, db_1.query)(`SELECT variant_id, external_product_id, external_variant_id
         FROM variant_publications
         WHERE platform = 'mercadolibre' AND variant_id IN (${placeholders})`, variantIds);
            for (const pr of pubRows || []) {
                const vid = String(pr.variant_id || '');
                const itemId = String(pr.external_product_id || '').trim();
                if (!vid || !itemId)
                    continue;
                if (!pubsByVariant.has(vid))
                    pubsByVariant.set(vid, []);
                const extVar = pr.external_variant_id;
        ...
        if (mlToken === null || mlToken === void 0 ? void 0 : mlToken.access_token) {
            ...
            const mlItemLinks = new Map();
            const pushMlLink = (itemId, link) => {
                const id = String(itemId || '').trim();
                if (!id)
                    return;
                if (!mlItemLinks.has(id))
                    mlItemLinks.set(id, []);
            ...
            for (const r of rows || []) {
                ...
                const baseLink = {
                    ...
                    color: String(r.color_name || r.color_code || '').trim(),
                    size: String(r.size_code || '').trim()
                };
                const pubs = pubsByVariant.get(variantId) || [];
                const linkCtx = variantLinkMeta.get(variantId);
                for (const pub of pubs) {
                    if (linkCtx &&
                        !shouldSyncMlPublication({ external_product_id: pub.itemId, external_variant_id: pub.variationId }, {
                            variantId,
                    ...
                    pushMlLink(pub.itemId, Object.assign(Object.assign({}, baseLink), { variationId: pub.variationId || (linkCtx === null || linkCtx === void 0 ? void 0 : linkCtx.ownVarId) || null }));
                }
                ...
                // Si solo hay pubs sin variation_id, sin esto ML queda en "—".
                {
                    const ownItemId = r.mercado_libre_item_id != null && String(r.mercado_libre_item_id).trim() !== ''
                        ? String(r.mercado_libre_item_id).trim()
                        : null;
                    const mlItemId = ownItemId || String(r.mercado_libre_id || '').trim();
                    const mlVarId = r.mercado_libre_variant_id != null && String(r.mercado_libre_variant_id).trim() !== ''
                        ? String(r.mercado_libre_variant_id).trim()
                        : null;
                    if (mlItemId) {
                        pushMlLink(mlItemId, Object.assign(Object.assign({}, baseLink), { variationId: mlVarId }));
                    }
                }
            }
            for (const [itemId, links] of mlItemLinks) {
                try {
                    const itemRes = yield axios_1.default.get(`https://api.mercadolibre.com/items/${encodeURIComponent(itemId)}?include_attributes=all`, { headers: mlHeaders, validateStatus: () => true });
                    if (itemRes.status !== 200 || !itemRes.data)
                        continue;
                    const item = itemRes.data;
                    for (const link of links) {
                        const qty = (0, mlVariationMatch_1.resolveMlStockForVariantLink)(item, link);
                        if (qty !== undefined)
                            stocks[link.variantId].stockML = qty;
                    }
                }
                catch (_c) {
...
const getMercadoLibreItemVariations = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const requestLooksLikeCatalog = /\/p\/MLA/i.test(requestRaw) ||
        ...
        if (catalogItemCandidates.length > 1) {
            ...
            const catalogVariations = Object.values(byItemId).map((it) => {
                ...
                const reconciled = (0, mlVariationMatch_1.reconcileMlColorSizeWithLupoSku)(sku, colorFromAttr || titleParsed.color || '', sizeFromAttr || titleParsed.size || '');
                return {
                    variationId: it.id,
                    itemId: String(it.id || ''),
                    sku,
...
const getTiendaNubeProductVariants = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        while (hasMoreVariants) {
            const variantsRes = yield axios_1.default.get(`https://api.tiendanube.com/v1/${storeId}/products/${productId}/variants`, { headers, params: { page: vPage, per_page: perPage, _: cacheBust }, validateStatus: () => true });
